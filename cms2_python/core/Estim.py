...
def likelihood_estimation(obs_color, init_alpha, fg_mu, bg_mu, fg_covariance, bg_covariance,
   ...
   #     fg_cov_inv = np.linalg.inv(fg_covariance)
    ...
    while (iteration_count <= max_iter) and (likelihood_diff > likelihood_min):
        ...
        SMU = np.dot(fg_cov_inv, fg_mu) + (obs_color.T * alpha / sigma_c_sq)
        SMD = np.dot(bg_cov_inv, bg_mu) + (obs_color.T * (1 - alpha) / sigma_c_sq)
        SM = np.vstack([SMU.T, SMD.T])
        ...
        Foreground_update = FB_matrix[0:3]
        Background_update = FB_matrix[3:6]
        
        alpha_numerator = np.dot((obs_color - Background_update).T, (Foreground_update - Background_update))
        alpha_denominator = sum(np.power((Foreground_update - Background_update), 2))[0]
        ...
        if iteration_count == 1:
            likelihood_diff = abs(total_likelihood[0][0])
        else:
            likelihood_diff = abs(total_likelihood[0][0] - prev_likelihood[0][0])
